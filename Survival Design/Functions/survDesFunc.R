...
survivalDesign <- function(alpha,power,tails,HR,intFrac,intPow,nSite,rpm,openRate,maxTime,penal,folUp,S_t,plotDes=T,results=T){

#############
### Sample Size estimate - Somewhere just over 500 events

if(tails==2) events <- gsdesign.survival(intFrac,HR,alpha,power,delta.eb=intPow)
if(tails==1) events <- gsdesign.survival(intFrac,HR,alpha,power,delta.eb=intPow,alternative="one.sided")
